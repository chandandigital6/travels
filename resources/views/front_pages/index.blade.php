   ...
   <div class="container-fluid destination py-5">
       <div class="container py-5">
           ...
           <div class="tab-class text-center">
               <ul class="nav nav-pills d-inline-flex justify-content-center mb-5">
                   <li class="nav-item">
                       <a class="d-flex mx-3 py-2 border border-primary bg-light rounded-pill" data-bs-toggle="pill" href="#tab-all">
                           <span class="text-dark" style="width: 150px;">All</span>
                       </a>
                   </li>
                   @foreach($destinations as $destination)
               ...
               <div class="tab-content">
                   <!-- All Destinations Tab Pane -->
                   <div id="tab-all" class="tab-pane fade show p-0">
                       <div class="row g-4">
                           @foreach($destinations as $destination)
                               @foreach($destination->states as $state)
                                   <div class="col-lg-6">
...
{{--                                                    <a href="#" class="btn-hover text-white">View All Place <i class="fa fa-arrow-right ms-2"></i></a>--}}
                ...
                @foreach($clientLogos->groupBy('title')->take(5) as $title => $logos)
                    <div id="GalleryTab-{{ Str::slug($title) }}" class="tab-pane fade p-0">
                        <div class="row g-2">
                            @foreach($logos->take(5) as $logo)
                                @if (is_string($logo->image) && !empty($logo->image))
                                    @php
                                        $imagePaths = explode(',', $logo->image);
                                    @endphp
                    ...
                    <div class="blog-item">
                        <div class="blog-img">
                            <div class="blog-img-inner">
                                <img class="img-fluid w-100 rounded-top" src="{{ asset('storage/'.$blog->image) }}"
                                    alt="Image">
                                <div class="blog-icon">
                            ...
                            <div class="blog-info d-flex align-items-center border border-start-0 border-end-0">
                                <small class="flex-fill text-center border-end py-2"><i
                                        class="fa fa-calendar-alt text-primary me-2"></i>{{ \Carbon\Carbon::parse($blog->created_at)->format('d, F Y') }}</small>
